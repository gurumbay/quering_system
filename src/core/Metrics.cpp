...
Metrics::Metrics() 
    : arrived_(0), refused_(0), completed_(0), sum_time_in_system_(0.0), 
      sum_waiting_time_(0.0), sum_service_time_(0.0) {
}
...
void Metrics::record_completion(size_t request_id, double time_in_system, double waiting_time, double service_time) {
    ++completed_;
...
double Metrics::get_device_utilization(size_t device_id, double total_time) const {
    if (total_time <= 0.0 || device_id >= device_busy_times_.size()) return 0.0;
    return device_busy_times_[device_id] / total_time;
}

size_t Metrics::get_arrived() const {
    return arrived_;
}

size_t Metrics::get_refused() const {
    return refused_;
}

size_t Metrics::get_completed() const {
    return completed_;
}
