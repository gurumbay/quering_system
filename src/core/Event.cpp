...
Event::Event(double time, EventType type, size_t request_id, size_t device_id, size_t source_id)
    : time_(time), type_(type), request_id_(request_id), device_id_(device_id), source_id_(source_id)
{}
...
size_t Event::get_device_id() const {
    return device_id_;
}

size_t Event::get_source_id() const {
    return source_id_;
}
