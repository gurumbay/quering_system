...
void TimelineCanvas::setZoomLevel(double zoom) {
  zoomLevel_ =
      std::max(0.1, std::min(zoom, 10.0));  // Limit zoom range 0.1x to 10x
  updateSize();
...
void TimelineCanvas::drawTimeline(QPainter& painter) {
  ...
  std::map<size_t, std::vector<std::pair<size_t, std::pair<double, double>>>>
      bufferOccupancy;
  ...
  for (const auto& event : events) {
    if (event.type == "service_start") {
      ...
      bufferOccupancy[event.buffer_slot].push_back(
          {event.request_id, {event.time, -1.0}});
  ...
  for (const auto& event : events) {
    ...
    int eventX =
        MARGIN_LEFT + static_cast<int>((drawWidth * event.time) / maxTime);

    if (event.type == "arrival") {
      int eventY =
          MARGIN_TOP + 40 + static_cast<int>(event.source_id) * LINE_HEIGHT;
      painter.setPen(QPen(Qt::red, 2));
  ...
  for (const auto& [device_id, services] : deviceServices) {
    ...
    for (const auto& [start, end] : services) {
      ...
      if (end < 0) {
        x2 =
            MARGIN_LEFT + static_cast<int>((drawWidth * currentTime) / maxTime);
        painter.setPen(QPen(Qt::darkGreen, 1, Qt::DashLine));
        painter.setBrush(QBrush(Qt::green, Qt::Dense4Pattern));
      } else {
        x2 = MARGIN_LEFT +
             static_cast<int>((drawWidth * std::min(end, maxTime)) / maxTime);
...
TimelineWidget::TimelineWidget(QWidget* parent) : QWidget(parent), timelineObserver_(nullptr) {
  ...
  auto addLegendItem = [&](const QString& text, const QColor& color,
    ...
    if (style == "arrival") {
      icon->setStyleSheet(QString("QLabel { background: %1; border: 1px solid "
                                  "%1; border-radius: 8px; }")
                              .arg(color.name()));
    } else if (style == "service") {
      icon->setStyleSheet(
          QString("QLabel { background: %1; border: 1px solid darkgreen; }")
              .arg(color.name()));
    } else if (style == "buffer") {
      icon->setStyleSheet(
          QString("QLabel { background: %1; border: 1px solid black; }")
              .arg(color.name()));
      ...
      icon->setStyleSheet(QString("QLabel { border-bottom: 3px dashed %1; }")
                              .arg(color.name()));
    }
  ...
  legendWidget->setStyleSheet(
      "QWidget { background: palette(base); border-top: 1px solid "
      "palette(mid); }");
  ...
  connect(canvas_, &TimelineCanvas::zoomChanged, this,
          &TimelineWidget::onCanvasZoomChanged);
...
void TimelineWidget::scrollToCurrentTime() {
  ...
  int drawWidth =
      canvasWidth - TimelineCanvas::MARGIN_LEFT - TimelineCanvas::MARGIN_RIGHT;
  int currentX = TimelineCanvas::MARGIN_LEFT +
                 static_cast<int>((drawWidth * currentTime) / maxTime);
  ...
  targetScrollX = std::max(
      0,
      std::min(targetScrollX, scrollArea_->horizontalScrollBar()->maximum()));
